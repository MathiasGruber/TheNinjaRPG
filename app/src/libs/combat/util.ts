...
/**
 * Get the most recent round when any jutsu with shared cooldown tags was used
 */
export const getLastUsedRoundForSharedCooldown = (actions: CombatAction[]): number => {
  return actions.reduce((acc, action) => {
    return Math.max(acc, action.lastUsedRound ?? -1);
  }, -1);
};
