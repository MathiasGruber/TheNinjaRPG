...
const Bank: NextPage = () => {
  ...
  // Schemas
  const toBankSchema = z.object({
    amount: z.number().int().positive().max(money),
  });
  const toPocketSchema = z.object({
    amount: z.number().int().positive().max(bank),
  });

  // Forms
  const toBankForm = useForm<z.infer<typeof toBankSchema>>({
    resolver: zodResolver(toBankSchema),
  });
  const toPocketForm = useForm<z.infer<typeof toPocketSchema>>({
    resolver: zodResolver(toPocketSchema),
  });
  ...
  if (l1 || l2) return <Loader explanation="Transferring money" />;
