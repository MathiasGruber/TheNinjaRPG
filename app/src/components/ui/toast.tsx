...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      "fixed z-100 flex max-h-screen w-full p-4 top-0 right-0 bottom-auto flex-col md:max-w-[420px]",
      className,
    )}
    {...props}
  />
...
>(({ className, variant, ...props }, ref) => {
  return (
    <ToastPrimitives.Root
      ref={ref}
      className={cn(toastVariants({ variant }), className, "mb-1")}
      {...props}
    />
  );
});
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Title
    ref={ref}
    className={cn("text-sm font-semibold [&+div]:text-xs md:block hidden", className)}
    {...props}
  />
));
