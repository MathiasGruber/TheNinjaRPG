...
export const jutsuRouter = createTRPCRouter({
      ...
      const filtered = results.filter((oneJutsu) => {
        if (
          ...
          return oneJutsu.effects.some((e) => {
            ...
            return (
              (!input.stat ||
                input.stat.every((x) => effectStats.includes(x))) &&
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (userjutsuObj) {
        await ctx.drizzle
          ...
          .where(and(eq(userJutsu.id, userjutsuObj.id), eq(userJutsu.userId, ctx.userId)));
      } else {
  ...
  toggleEquip: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const loadout = loadouts.find((l) => l.id === user.jutsuLoadout);
      const isLoaded =
        userjutsuObj && loadout?.jutsuIds.includes(userjutsuObj.jutsuId);
      const residualJutsus = userjutsus.filter(
        (uj) =>
          uj.equipped &&
          uj.jutsu.effects.some((e) => "residualModifier" in e && e.residualModifier),
      ...
      if (loadout && isLoaded && newEquippedState === 0) {
        loadout.jutsuIds = loadout.jutsuIds.filter(
          (id) => id !== userjutsuObj.jutsuId,
        );
  ...
  return [
    ...
    ...(input?.effect?.length
      ? [
          or(
            ...input.effect.map(
              (e) => sql`JSON_SEARCH(${jutsu.effects}, 'one', ${e}, NULL, '$[*].type') IS NOT NULL`,
            ),
          ),
        ]
      : []),
    ...
    ...(input?.excludedJutsuTypes?.length
      ? input.excludedJutsuTypes.map((excludedType) =>
          ne(jutsu.jutsuType, excludedType as "NORMAL" | "EVENT" | "CLAN" | "SPECIAL" | "BLOODLINE" | "FORBIDDEN" | "LOYALTY" | "AI"),
        )
      : []),
    ...(input?.excludedClassifications?.length
      ? input.excludedClassifications.map((c) =>
          ne(jutsu.statClassification, c as "Highest" | "Ninjutsu" | "Genjutsu" | "Taijutsu" | "Bukijutsu"),
        )
      : []),
    ...
    ...(input?.excludedRanks?.length
      ? input.excludedRanks.map((r) =>
          ne(jutsu.requiredRank, r as "STUDENT" | "GENIN" | "CHUNIN" | "JONIN" | "COMMANDER" | "ELDER" | "NONE"),
        )
      : []),
    ...(input?.excludedMethods?.length
      ? input.excludedMethods.map((m) =>
          ne(jutsu.method, m as "ALL" | "SINGLE" | "AOE_CIRCLE_SPAWN" | "AOE_LINE_SHOOT" | "AOE_WALL_SHOOT" | "AOE_CIRCLE_SHOOT" | "AOE_SPIRAL_SHOOT"),
        )
      : []),
    ...(input?.excludedTargets?.length
      ? input.excludedTargets.map((t) =>
          ne(jutsu.target, t as "SELF" | "OTHER_USER" | "OPPONENT" | "ALLY" | "CHARACTER" | "GROUND" | "EMPTY_GROUND"),
        )
      : []), 
