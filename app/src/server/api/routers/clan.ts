...
export const clanRouter = createTRPCRouter({
  purchaseHideout: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      // Fetch
      const [user, fetchedClan, villages] = await Promise.all([
      ...
      const result = await ctx.drizzle
        ...
        .set({
          villageId: hideoutId,
          ryoTreasury: sql`${clan.ryoTreasury} - ${HIDEOUT_COST}`,
        })
        .where(and(eq(clan.id, input.clanId), gte(clan.ryoTreasury, HIDEOUT_COST)));
      if (result.rowsAffected === 0) return errorResponse("Failed to purchase hideout");
      // Mutate step 2 - set hideout
      await ctx.drizzle.insert(village).values({
        id: hideoutId,
        name: `${fetchedClan.name} Hideout`,
        mapName: `${fetchedClan.name} Hideout`,
        sector: input.sector,
  ...
  upgradeHideoutToTown: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (clanHideout.wasDowngraded) {
        const missing = TOWN_REESTABLISH_COST - fetchedClan.ryoTreasury;
        if (missing > 0) {
      ...
      const result = await ctx.drizzle
        ...
        .set({
          ...(clanHideout.wasDowngraded
            ? { ryoTreasury: sql`${clan.ryoTreasury} - ${TOWN_REESTABLISH_COST}` }
        ...
        .where(
          and(
            ...
            ...(clanHideout.wasDowngraded
              ? [gte(clan.ryoTreasury, TOWN_REESTABLISH_COST)]
  ...
  clanDonate: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (user.money < input.ryo) {
        return errorResponse("You do not have enough ryo in your pocket");
      }
      if (!(input.ryo > 0 || input.reputationPoints > 0)) {
        return errorResponse("Either ryo or reps must be donated.");
      }
      ...
      const result = await ctx.drizzle
        .update(userData)
        .set({
          money: sql`${userData.money} - ${input.ryo}`,
          reputationPoints: sql`${userData.reputationPoints} - ${repsCost}`,
        })
        .where(
          and(
            eq(userData.userId, ctx.userId),
            gte(userData.money, input.ryo),
      ...
      const result2 = await ctx.drizzle
        .update(clan)
        .set({
          ryoTreasury: sql`${clan.ryoTreasury} + ${input.ryo}`,
      ...
      if (result2.rowsAffected === 0) {
        ...
        if (input.ryo > 0) donates.push(`${input.ryo} ryo`);
        if (repsCost > 0) donates.push(`${repsCost} reps`);
        const message = `${user.username} donated: ${donates.join(", ")} to faction`;
