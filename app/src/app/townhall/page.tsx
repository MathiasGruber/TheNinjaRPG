...
export default function TownHall() {
  ...
  const availableTabs = ["Alliance", "Kage", "Elders", "War Room"] as const;
...
const WarRoom: React.FC<{
  ...
  const { mutate: hireFaction } = api.war.hireFaction.useMutation({
    onSuccess: async (data) => {
      showMutationToast(data);
      if (data.success) {
        await utils.war.getActiveWars.invalidate();
  ...
  return (
    <>
      ...
      >
        <p>
          The War Room is where the Kage makes critical decisions about village warfare.
