import { type PrismaClient, type ForumBoard } from "@prisma/client";
...
const upsertBoard = async (
  ...
  // Database call
  const obj = await prisma.forumBoard.upsert({
    where: {
      name: board.name,
    },
    update: {},
...
export const seedForum = async (prisma: PrismaClient) => {
  ...
  const promises: Promise<ForumBoard>[] = [];
  for (const board of boards) {
    promises.push(upsertBoard(prisma, board));
  }
